...
export class ChatController{
  ...
  _bindEvents(){const{launcher,input,sendBtn,clearBtn,closeBtn}=this._ui;launcher.addEventListener('click',()=>this._toggleOpen());if(closeBtn)closeBtn.addEventListener('click',()=>this._close());document.addEventListener('keydown',(e)=>{if(e.key==='Escape'&&this._isOpen)this._close();});document.addEventListener('click',(e)=>{if(!this._isOpen)return;const path=e.composedPath();if(!path.includes(this._ui.host))this._close();},{capture:true});input.addEventListener('input',()=>{this._ui.resizeInput();sendBtn.disabled=input.value.trim()===''||this._isBusy;});input.addEventListener('keydown',(e)=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();if(!sendBtn.disabled)this._sendMessage();}});sendBtn.addEventListener('click',()=>{if(!sendBtn.disabled)this._sendMessage();});clearBtn.addEventListener('click',()=>{this._history=[];clearHistory();this._ui.clearMessages();});}
