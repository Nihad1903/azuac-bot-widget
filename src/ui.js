...
function tmpl(title){return'<style>'+cssText+'</style>'+
  ...
  '<div class="uni-chat-window uni-chat-hidden" id="uni-chat-window" role="dialog" aria-modal="true">'+
    '<div class="uni-chat-header">'+
      '<div class="uni-chat-header-avatar">'+IB+'</div>'+
      '<div class="uni-chat-header-info">'+
        '<div class="uni-chat-header-title">'+esc(title)+'</div>'+
        '<div class="uni-chat-header-status"><span class="uni-chat-status-dot"></span>Online</div>'+
      '</div>'+
      '<button class="uni-chat-clear-btn" id="uni-chat-clear-btn">Clear</button>'+
    '</div>'+
    '<div class="uni-chat-messages" id="uni-chat-messages" role="log" aria-live="polite">'+
      '<div class="uni-chat-empty" id="uni-chat-empty">'+
        '<div class="uni-chat-empty-icon">\u{1F4AC}</div>'+
        '<h3>How can I help you?</h3>'+
        '<p>Ask me anything. I am here to assist you.</p>'+
      '</div>'+
    '</div>'+
    '<div class="uni-chat-footer">'+
      '<div class="uni-chat-input-row">'+
        '<textarea class="uni-chat-input" id="uni-chat-input" placeholder="Type your message\u2026" rows="1"></textarea>'+
        '<button class="uni-chat-send-btn" id="uni-chat-send-btn" disabled>'+IS+'</button>'+
      '</div>'+
      '<p class="uni-chat-footer-note">Powered by AZUAC</p>'+
    '</div>'+
  '</div>';}
export function createUserMessageEl(t,ts=new Date()){
  ...
  return w;}
export function createBotMessageEl(t,ts=new Date(),isError=false){
  const w=document.createElement('div');w.className='uni-chat-msg uni-chat-msg-bot'+(isError?' uni-chat-msg-error':'');
  w.innerHTML='<div class="uni-chat-msg-row"><div class="uni-chat-bot-avatar">'+(isError?IE:IB)+'</div><div class="uni-chat-msg-bubble">'+esc(t)+'</div></div><time class="uni-chat-msg-time" datetime="'+ts.toISOString()+'">'+fmt(ts)+'</time>';
  return w;}
export function createTypingIndicatorEl(){
  ...
  el.innerHTML='<div class="uni-chat-typing-avatar">'+IB+'</div><div class="uni-chat-typing-dots"><span></span><span></span><span></span></div>';
  return el;}
